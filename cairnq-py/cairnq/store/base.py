...
class TaskStore(ABC):
    ...
    async def _has_claimable_work(self, params: dict[str, Any]) -> bool:
        ...
        read-only `claimable_probe`, which every dialect ships.

        Both dialects want it, for different reasons: SQLite so an idle worker
        never takes its single write lock and idle workers stop serializing
        against each other, Postgres so an empty poll costs one statement instead
        of a transaction plus `recover_leases` plus one claim statement per
        self-limiting name. Neither reason is dialect-specific enough to live in
        a dialect: what differs is the SQL, which is where the protocol keeps
        dialect differences already. A backend whose probe would cost more than
        the claim it guards overrides this with `return True`."""
