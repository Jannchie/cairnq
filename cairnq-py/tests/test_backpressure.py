...
# ------------------------------------------------------------------ queue_depth
async def test_reports_headroom_saturating_at_zero(client):
    assert await client.queue_depth("default", 3) == 3
    await client.submit("job", {})
    assert await client.queue_depth("default", 3) == 2
    await client.submit("job", {})
    await client.submit("job", {})
    assert await client.queue_depth("default", 3) == 0
...
async def test_counts_queued_only_so_a_claim_frees_headroom(client):
    await client.submit("job", {})
    assert await client.queue_depth("default", 5) == 4
    ...
    assert await client.queue_depth("default", 5) == 5


async def test_counts_delayed_tasks_they_are_queued_work_that_will_run(client):
    await client.submit("job", {}, run_at_delay_ms=60_000)
    assert await client.queue_depth("default", 5) == 4


async def test_depth_is_per_queue(client):
    await client.submit("job", {}, queue="a")
    await client.submit("job", {}, queue="a")
    assert await client.queue_depth("a", 5) == 3
    assert await client.queue_depth("b", 5) == 5
