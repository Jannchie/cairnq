...
from cairnq.retention import DEFAULT_LIMIT, RetentionSweeper
...
async def test_deletes_terminal_tasks_past_the_cutoff(backend):
    ...
    sweeper = RetentionSweeper(client.store, 0, interval_ms=20)
    sweeper.start()
    try:
...
async def test_keeps_tasks_that_have_not_aged_out(backend):
    ...
    sweeper = RetentionSweeper(client.store, 3_600_000, interval_ms=20)
    sweeper.start()
    try:
...
async def test_does_not_purge_on_startup(backend):
    ...
    client = await backend.client(retention_ms=0)
    done = await _finish_one(client)
...
async def test_stops_on_close_without_leaving_a_purge_behind(backend):
    ...
    sweeper = RetentionSweeper(client.store, 0, interval_ms=10)
    sweeper.start()
    await _wait_for(lambda: _is_gone(client, done))
...
async def test_keeps_sweeping_after_a_sweep_that_threw(backend):
    ...
    sweeper = RetentionSweeper(store, 0, interval_ms=20)
    sweeper.start()
    try:
...
async def test_refuses_a_cutoff_or_interval_that_cannot_mean_anything(backend):
    with pytest.raises(ValueError, match="retention_ms"):
        await backend.client(retention_ms=-1)
    client = await backend.client()
    with pytest.raises(ValueError, match="interval_ms"):
        RetentionSweeper(client.store, 0, interval_ms=0)
...
async def test_drains_a_backlog_larger_than_one_statement():
    sweeper = RetentionSweeper(_ScriptedStore(DEFAULT_LIMIT, DEFAULT_LIMIT, 7), 0)
...
async def test_stops_promptly_after_an_on_demand_sweep():
    ...
    sweeper = RetentionSweeper(_ScriptedStore(DEFAULT_LIMIT), 0, interval_ms=3_600_000)
    sweeper.start()
    await sweeper.sweep()
...
async def test_a_stopped_sweeper_still_drains_on_demand():
    ...
    been skipped."""
    sweeper = RetentionSweeper(_ScriptedStore(DEFAULT_LIMIT, 5), 0, interval_ms=20)
    sweeper.start()
    await sweeper.stop()
...
async def test_hands_the_loop_back_between_batches_of_a_drain_after_a_stop():
    ...
    sweeper = RetentionSweeper(_ScriptedStore(DEFAULT_LIMIT, 2), 0, interval_ms=20)
    sweeper.start()
    await sweeper.stop()
