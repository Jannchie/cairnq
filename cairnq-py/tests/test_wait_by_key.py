...
async def test_reattaches_by_id_without_rerunning(client):
...
async def test_reattaches_by_key(client):
...
async def test_follows_the_key_onto_a_replacement(client):
...
async def test_waits_for_a_key_that_points_at_nothing_yet(client):
    waiting = asyncio.create_task(client.wait_by_key("A", timeout_ms=3_000))
    await asyncio.sleep(0.03)
    await client.submit("job", {}, key="A")
...
async def test_names_the_key_it_was_watching(client):
    with pytest.raises(TaskTimeout) as caught:
